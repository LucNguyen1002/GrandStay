...
import { CircleCheck, Eye, EyeOff, LoaderCircle, LockKeyhole, RefreshCw, ShieldCheck, UserRound } from 'lucide-react'
...
export function LoginPage() {
  ...
  const submit = async (values: LoginForm) => {
    ...
    try {
      ...
      if (message === 'Không thể kết nối máy chủ.') restartBackendCheck()
    }
  }
  ...
  }, [backendReady, loginWithGoogle, navigate, requestedRoute, restartBackendCheck])
  ...
  return <AuthLayout eyebrow="Chào mừng trở lại" title="Đăng nhập GrandStay" description="Đăng nhập bằng tài khoản của bạn để tiếp tục.">
    <div role="status" aria-live="polite" className={`mt-6 rounded-2xl border px-4 py-3 ${backendReady ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold">{backendReady ? 'Máy chủ đã sẵn sàng' : backendStatus === 'slow' ? 'Máy chủ đang khởi động lâu hơn dự kiến' : 'Đang khởi động máy chủ…'}</p>
          {!backendReady && <p className="mt-0.5 text-xs leading-5 text-amber-800">Render Free có thể cần khoảng một phút. Trang sẽ tự kiểm tra và bật nút đăng nhập khi sẵn sàng.</p>}
        </div>
        {backendStatus === 'slow' && <button type="button" onClick={restartBackendCheck} className="inline-flex shrink-0 items-center gap-1 rounded-lg px-2 py-1 text-xs font-bold hover:bg-amber-100"><RefreshCw size={13}/>Kiểm tra ngay</button>}
      </div>
    </div>
    <form onSubmit={handleSubmit(submit)} className="mt-8 space-y-5">
      ...
      <Button type="submit" loading={isSubmitting} disabled={googleBusy || !backendReady} className="w-full py-3.5">{backendReady ? 'Đăng nhập' : 'Đang chờ máy chủ'}</Button>
    </form>
