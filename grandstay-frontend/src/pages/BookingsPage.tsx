  ...
  <Modal title={summary ? `${language === 'vi' ? 'Đặt phòng' : 'Booking'} ${summary.bookingNumber}` : (language === 'vi' ? 'Chi tiết đặt phòng' : 'Booking details')} size="xl" onClose={onClose}>
    {details.isLoading ? <Loading /> : details.error || !view || !summary ? <div role="alert" className="rounded-xl bg-red-50 p-4 text-sm text-red-700">{errorMessage(details.error)}</div> : <div className="space-y-5">
      ...
      {summary.status === 'CHECKED_IN' && can('service:write') && <div className="rounded-2xl border border-gold-soft bg-amber-50/40 p-4">
        ...
        <Button className="mt-3" disabled={!usage.bookingRoomId || !usage.serviceId || usage.quantity <= 0} loading={addUsage.isPending} onClick={() => addUsage.mutate()}>{text('Ghi nhận dịch vụ', 'Add service')}</Button>
      </div>}
...
function CreateBookingModal({ initialRoomId, initialRoomTypeId, initialCheckIn, initialCheckOut, initialGuests, selfService, customerName, onClose, onCreated }: { initialRoomId?: string; initialRoomTypeId?: string; initialCheckIn?: string; initialCheckOut?: string; initialGuests?: string; selfService: boolean; customerName: string; onClose: () => void; onCreated: (id: string) => void }) {
  const { language, money: formatCurrency, text } = useI18n()
  const { can } = useAuth()
  ...
  const mutation = useMutation({
    ...
    onSuccess: data => { toast.success(text(`Đã tạo đặt phòng gồm ${roomDrafts.length} phòng.`, `Booking created with ${roomDrafts.length} room${roomDrafts.length === 1 ? '' : 's'}.`)); onCreated(data.id) },
    onError: error => toast.error(errorMessage(error)),
  })
    ...
    <div className="space-y-6">
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {selfService ? <div className="sm:col-span-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900"><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">{text('Khách chính', 'Primary guest')}</span><p className="mt-1 font-bold">{customerName}</p><p className="mt-1 text-xs">{text('Đặt phòng sẽ được liên kết trực tiếp với tài khoản đang đăng nhập.', 'This booking will be linked directly to your signed-in account.')}</p></div> : <>
            <label className="sm:col-span-2"><span className="label">{text('Khách hàng có sẵn', 'Existing customer')}</span><select className="field" value={form.customerId} onChange={event => set('customerId', event.target.value)}><option value="">{text('Khách vãng lai / nhập tên mới', 'Walk-in guest / enter a new name')}</option>{customers.data?.map(customer => <option key={customer.id} value={customer.id}>{customer.customerCode} · {customer.fullName}</option>)}</select></label>
            {!form.customerId && <label className="sm:col-span-2"><span className="label">{text('Họ tên khách chính', 'Primary guest name')}</span><input autoFocus className="field" value={form.guestName} onChange={event => set('guestName', event.target.value)} required/></label>}
          </>}
          <label><span className="label">{text('Giờ nhận phòng dự kiến', 'Expected check-in')}</span><input type="datetime-local" className="field" value={form.checkIn} onChange={event => set('checkIn', event.target.value)}/></label>
          <label><span className="label">{text('Giờ trả phòng dự kiến', 'Expected check-out')}</span><input type="datetime-local" className={`field ${form.checkOut && !validPeriod ? 'field-error' : ''}`} value={form.checkOut} min={form.checkIn || undefined} onChange={event => set('checkOut', event.target.value)}/>{form.checkOut && !validPeriod && <small className="mt-1 block text-xs text-red-700">{text('Thời gian trả phòng phải sau thời gian nhận phòng.', 'Check-out must be after check-in.')}</small>}</label>
          {can('promotion:read') && <label className="sm:col-span-2"><span className="label">{text('Chương trình ưu đãi', 'Promotion')}</span><select className="field" value={form.promotionId} onChange={event => set('promotionId', event.target.value)}><option value="">{text('Không áp dụng ưu đãi', 'No promotion')}</option>{promotions.data?.map(item => <option key={item.id} value={item.id}>{item.code} · {item.name} · {item.discountType === 'PERCENTAGE' ? text(`giảm ${item.discountValue}%`, `${item.discountValue}% off`) : text(`giảm ${formatCurrency(Number(item.discountValue))}`, `${formatCurrency(Number(item.discountValue))} off`)}</option>)}</select>{promotions.isLoading && <small className="mt-1 block text-xs text-ink-soft">{text('Đang tải ưu đãi hợp lệ…', 'Loading eligible promotions…')}</small>}{promotions.error && <small role="alert" className="mt-1 block text-xs text-red-700">{errorMessage(promotions.error)}</small>}</label>}
        </div>
      </section>

      <section className="border-t border-slate-100 pt-5">
        <div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="font-bold">{text('2. Phòng và số khách', '2. Rooms and guests')}</h3><p className="text-xs text-ink-soft">{text('Chỉ hiển thị phòng thực sự trống trong toàn bộ khoảng lưu trú.', 'Only rooms available for the entire stay are shown.')}</p></div><Button variant="secondary" disabled={!validPeriod} onClick={() => setRoomDrafts(previous => [...previous, { key: draftKey(), roomId: '', ratePlanId: '', adults: 1, children: 0 }])}><Plus size={15}/>{text('Thêm phòng', 'Add room')}</Button></div>
        {!validPeriod && <div className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">{text('Chọn thời gian nhận và trả phòng trước khi phân phòng.', 'Select check-in and check-out before assigning rooms.')}</div>}
        {availableRooms.isFetching && <p className="py-3 text-sm text-ink-soft">{text('Đang kiểm tra phòng trống…', 'Checking availability…')}</p>}
        ...
        <div className="space-y-3">{roomDrafts.map((draft, index) => {
          ...
          return <div key={draft.key} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
            <div className="mb-3 flex items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{text('Phòng', 'Room')} {index + 1}</strong>{selectedRoomType && <span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-800">{text(`Tối đa ${selectedRoomType.capacityAdults} người lớn · ${selectedRoomType.capacityChildren} trẻ em`, `Up to ${selectedRoomType.capacityAdults} adults · ${selectedRoomType.capacityChildren} children`)}</span>}</div>{roomDrafts.length > 1 && <button type="button" aria-label={text(`Xóa phòng ${index + 1}`, `Remove room ${index + 1}`)} className="rounded-lg p-2 text-red-600 hover:bg-red-50" onClick={() => setRoomDrafts(previous => previous.filter(room => room.key !== draft.key))}><Trash2 size={16}/></button>}</div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <label><span className="label">{text('Phòng', 'Room')}</span><select className={`field ${unavailable ? 'field-error' : ''}`} value={draft.roomId} disabled={!validPeriod} onChange={event => {
        ...
        <p className="mt-3 text-right text-sm text-ink-soft">{text('Tổng cộng', 'Total')}: <strong className="text-ink">{text(`${roomDrafts.length} phòng · ${adults} người lớn · ${children} trẻ em`, `${roomDrafts.length} rooms · ${adults} adults · ${children} children`)}</strong></p>
      </section>
      ...
      <label className="block border-t border-slate-100 pt-5"><span className="label">{text('Yêu cầu đặc biệt', 'Special requests')}</span><textarea className="field min-h-20" value={form.specialRequests} onChange={event => set('specialRequests', event.target.value)}/></label>
    </div>
    <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button variant="secondary" onClick={onClose}>{text('Đóng', 'Close')}</Button><Button disabled={!valid} loading={mutation.isPending} onClick={() => mutation.mutate()}>{selfService ? text('Xác nhận đặt phòng', 'Confirm booking') : text('Tạo và xác nhận đặt phòng', 'Create and confirm booking')}</Button></div>
  </Modal>
}
