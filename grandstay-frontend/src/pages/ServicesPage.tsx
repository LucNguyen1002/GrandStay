...
export function ServicesPage() {
  ...
  const remove = useMutation({ mutationFn: (id: string) => api.delete(`/services/${id}`), onSuccess: () => { toast.success(text('Đã xóa dịch vụ khỏi danh mục.', 'Service removed from the catalog.')); setDeleting(null); refresh() }, onError: e => toast.error(errorMessage(e)) })
  ...
  return <><PageHeader title={text('Danh mục dịch vụ', 'Service catalog')} description={text('Quản lý cả dịch vụ đang bán và tạm ngưng; chỉ dịch vụ đang bán mới xuất hiện khi ghi nhận cho khách.', 'Manage active and paused services. Only active services can be added to a guest stay.')} action={can('service:write') ? <Button onClick={() => setEditing('new')}><Plus size={17}/>{text('Thêm dịch vụ', 'Add service')}</Button> : undefined}/><Card><div className="table-shell"><table className="data-table"><thead><tr><th>{text('Mã', 'Code')}</th><th>{text('Dịch vụ', 'Service')}</th><th>{text('Nhóm', 'Category')}</th><th>{text('Đơn giá', 'Unit price')}</th><th>{text('Thuế', 'Tax')}</th><th>{text('Trạng thái', 'Status')}</th><th></th></tr></thead><tbody>{query.data?.map(item => <tr key={item.id} className={item.active ? '' : 'bg-slate-50/70'}><td className="font-bold">{item.code}</td><td>{item.name}<div className="text-xs text-ink-soft">/{language === 'en' && item.unit.toLowerCase() === 'lần' ? 'time' : item.unit}</div></td><td>{serviceCategoryLabel(item.category, language)}</td><td>{money(Number(item.unitPrice), item.currency)}</td><td>{item.taxRate}%</td><td><Badge tone={statusTone(item.active ? 'ACTIVE' : 'INACTIVE')}>{item.active ? text('Đang bán', 'Active') : text('Tạm ngưng', 'Paused')}</Badge></td><td><div className="flex justify-end gap-1">{can('service:write') && <>{!item.active && <button aria-label={`${text('Bật lại', 'Restore')} ${item.name}`} title={text('Bật lại dịch vụ', 'Restore service')} className="rounded-lg p-2 text-emerald-700 hover:bg-emerald-50" disabled={restore.isPending} onClick={() => restore.mutate(item)}><RotateCcw size={17}/></button>}<button aria-label={`${text('Sửa', 'Edit')} ${item.name}`} className="rounded-lg p-2 hover:bg-slate-100" onClick={() => setEditing(item)}><Pencil size={17}/></button><button aria-label={`${text('Xóa', 'Delete')} ${item.name}`} className="rounded-lg p-2 text-red-600 hover:bg-red-50" onClick={() => setDeleting(item)}><Trash2 size={17}/></button></>}</div></td></tr>)}</tbody></table>{!query.data?.length && <Empty/>}</div></Card>{editing && <ServiceModal service={editing === 'new' ? undefined : editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); refresh() }}/>} {deleting && <ConfirmDialog title={text('Xóa dịch vụ khỏi danh mục?', 'Remove service from catalog?')} description={text(`Dịch vụ “${deleting.name}” sẽ không còn xuất hiện trong danh mục. Dữ liệu đã phát sinh vẫn được giữ nguyên.`, `“${deleting.name}” will no longer appear in the catalog. Existing transaction data will be preserved.`)} confirmLabel={text('Xóa dịch vụ', 'Remove service')} loading={remove.isPending} onCancel={() => setDeleting(null)} onConfirm={() => remove.mutate(deleting.id)}/>}</>
}
