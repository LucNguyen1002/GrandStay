...
export function RegisterPage() {
  ...
  return <AuthLayout eyebrow={t('auth.registerEyebrow')} title={t('auth.registerTitle')} description={t('auth.registerDescription')}>
    <form onSubmit={handleSubmit(submit)} className="mt-7 space-y-4">
      ...
      <label className="block"><span className="label">{t('auth.password')}</span><div className="relative"><LockKeyhole className="field-icon" size={18}/><input type={showPassword ? 'text' : 'password'} autoComplete="new-password" className={`field field-with-icon field-with-action ${errors.password ? 'field-error' : ''}`} {...register('password')} /><button type="button" className="field-action" aria-label={showPassword ? text('Ẩn mật khẩu', 'Hide password') : text('Hiện mật khẩu', 'Show password')} onClick={() => setShowPassword(value => !value)}>{showPassword ? <EyeOff size={18}/> : <Eye size={18}/>}</button></div>{errors.password && <small className="mt-1 block text-red-700">{errors.password.message}</small>}</label>
