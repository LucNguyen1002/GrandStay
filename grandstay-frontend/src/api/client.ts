...
export function errorMessage(error: unknown): string {
  ...
  if (axios.isAxiosError<ProblemDetail | string>(error)) {
    ...
    const translatedDetail: Record<string, string> = {
      ...
      'Identity verification is required before check-in': 'Khách chính cần hoàn tất và xác minh thông tin danh tính trước khi nhận phòng.',
      'A verified customer profile is required before check-in': 'Cần liên kết hồ sơ khách hàng đã xác minh trước khi nhận phòng.',
      ...
      'Customer deposit would exceed the allowed amount': 'Tổng tiền cọc vượt quá giá trị lưu trú dự kiến.',
      'A deposit cannot be recorded for this booking status': 'Không thể ghi nhận tiền cọc ở trạng thái đặt phòng hiện tại.',
    ...
    const englishDetail: Record<string, string> = {
      ...
      'A verified customer profile is required before check-in': 'A verified customer profile is required before check-in.',
