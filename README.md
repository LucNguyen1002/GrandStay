...
2. Cấu hình `POSTGRES_PASSWORD`, `JWT_SECRET` và `ADMIN_BOOTSTRAP_PASSWORD` bằng giá trị bí mật mạnh. Giữ `ADMIN_BOOTSTRAP_ENABLED=true` ở lần chạy đầu. Để bật đăng nhập Google, điền Web Client ID vào `GOOGLE_CLIENT_ID`.
...
Tài khoản có vai trò `CUSTOMER` có thể chọn phòng trống tại **Sơ đồ phòng**, tạo booking, xem thông tin tiền cọc và xem hoặc hủy booking của chính mình. Backend liên kết tài khoản với một hồ sơ `Customer` qua `customers.user_id`; các API `/api/v1/self/**` không chấp nhận định danh chủ sở hữu do trình duyệt cung cấp và không trả booking hoặc payment của tài khoản khác.
...
- `compose.yml`: PostgreSQL, backend và frontend Nginx với health checks và persistent volume.
